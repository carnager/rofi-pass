...
* Run arbitrary shell command to get a password (makes arbitrary in-house integration possible)
...
Rofi-pass allows you to use arbitrary shell commands in fields of a
password entry.  This can be useful if in your organization you use
some different authorization element than passwords or supported OTP
types.

E.g. my bank uses one-time passwords generated by plugging my bank
card into the smartcard slot of my computer and then running a binary
program to get the next OTP out of the chipcard.

I can support this in my rofi-pass configuration like this:

```
mybankpassword
---
user: mybankuser
run_command_chipcard: /usr/local/bin/bank-smartcard-otp-gen
autotype: user :tab pass :tab run_command_chipcard
```

In the above example, the "chipcard" is just an arbitrary name up to
your decision, rofi-pass only parses the `run_command` part of the string.
Therefore you can even have multiple `run_command` fields:

```
sillyexample
---
run_command_date: date
run_command_count: cat /etc/fstab | wc -l
autotype: pass :enter run_command_date :enter run_command_count
```
